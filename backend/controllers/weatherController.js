...
const generateDate = function(daysFromNow) {
  const date = new Date();
  ...
  getAllWeather: function(req, res) {
    try {
      res.json({
        ...
        data: weatherData.regions,
        monsoon_forecast: weatherData.monsoon_forecast,
  ...
  getRegionWeather: function(req, res) {
    try {
      ...
      const regionData = weatherData.regions.find(r =>
        r.name.toLowerCase().replace(/\s+/g, '-') === region.toLowerCase().trim() ||
        r.name.toLowerCase() === region.toLowerCase().trim().replace(/-/g, ' ')
      );
